function [u0, J0, duration] = compute_controlXM(A, X0, B, u0, Q, R, xd, tgrid, batches, pim, Mass)
...
for ii = 1:max_iters
    xsol0 = compute_XM(A, X0, B, u0, tgrid, batches, pim, Mass);
    J0 = cost_function(Q,R,xd,xsol0,u0,tgrid,dt);
    
    phi = compute_phiM(A, Q, xsol0, xd, tgrid, batches, pim, Mass);
    gradJ = B.'*phi + 2*R*u0;
    G = innerproduct(gradJ,gradJ,dt);
    
    dx = compute_XM(A, 0*X0, B, gradJ, tgrid, batches, pim, Mass);
...
%             xsol1 = compute_XM(A, X0, B, u1, tgrid, batches, pim, Mass);
